    ...
    kids=[
        StringFileInfo([
            StringTable(
                '040904B0',
                [
                    StringStruct('CompanyName', 'NovaNectar'),
                    StringStruct('FileDescription', 'NN Billing System'),
                    StringStruct('FileVersion', '1.0.0.0'),
                    StringStruct('InternalName', 'NN_Billing_System'),
                    StringStruct('LegalCopyright', '© 2025 NovaNectar'),
                    StringStruct('OriginalFilename', 'NN_Billing_System.exe'),
                    StringStruct('ProductName', 'Nova Billing'),
...
customer_id = 1       # 🛑 Yaha apne customer ke actual ID daalo
amount = 500.0
date = datetime.now().strftime("%Y-%m-%d")
status = "paid"       # ya "pending"

# Step 3: Insert into table
cursor.execute("INSERT INTO invoices_table (customer_id, amount, date, status) VALUES (?, ?, ?, ?)",
...
print("✅ Invoice record added successfully.")
...
def check_license(email, password):
    ...
    try:
        ...
        if data['status'] == 'active':
            ...
            print("❌", data["message"])
            print("💳 Please make a payment to reactivate your license:")
            print("Bank Name :", data["payment_info"]["bank_name"])
            print("Account No:", data["payment_info"]["account_no"])
            print("UPI ID    :", data["payment_info"]["upi"])

        elif data['status'] == 'deactive':
            print("🚫", data["message"])
...
def on_signin():
    ...
    if check_license(email, password):
        messagebox.showinfo("Success", "Login successful!")
        # proceed to app...
    else:
        messagebox.showerror("Access Denied", "License check failed.")
...
def resource_path(relative_path):
    ...
    try:
        base_path = sys._MEIPASS  # Used when running as a bundle
    except Exception:
        base_path = os.path.abspath(".")
    return os.path.join(base_path, relative_path)
...
def open_invoice_image(
    ...
    # Load logo
    try:
        logo = Image.open(resource_path("logo2.jpg")).convert("RGBA")
...
def show_dashboard_view():
    ...
    card_frame.grid(row=1, column=0, columnspan=3, pady=10)
